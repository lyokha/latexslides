...
""")
    ...
    def add_slide(self, slide):
        ...
        elif isinstance(slide, SubSection):
            if not self.sections:
                raise TypeError, "no section defined prior to subsection"
            #if not self.sections[-1].subsections:
            self.sections[-1].subsections.append(slide)
            #else:
            #    self.sections[-1].subsections[-1].add_slide(sect)
        elif isinstance(slide, (tuple, list)):
            raise TypeError, "slide cannot be tuple, remove comma"
        if not self.sections:
            self.slides.append(slide)
        else:
            if not self.sections[-1].subsections:
                self.sections[-1].add_slide(slide)
            else:
                self.sections[-1].subsections[-1].add_slide(slide)
